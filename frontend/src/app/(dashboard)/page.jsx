...
export default function DashboardPage() {
  ...
  const dashboardData = useMemo(() => {
    ...
    financeiroFiltrado.forEach(lanc => {
      ...
      // Normaliza o tipo: remove espaços e converte para minúsculo
      const tipo = (lanc.tipo || "").trim().toLowerCase();

      if (!dailyMap[dateKey]) dailyMap[dateKey] = { receita: 0, despesa: 0 };

      // Lógica Robusta: Se for Entrada ou Receita, soma na receita.
      // Caso contrário (else), assume que é uma Saída/Despesa.
      // Isso resolve problemas de acentuação ('í' vs 'i') e variações de escrita.
      if (tipo === 'entrada' || tipo === 'receita') {
        dailyMap[dateKey].receita += val;
      } else {
        dailyMap[dateKey].despesa += val;
      }
    });
                ...
                dashboardData.recentSolicitacoes.length > 0 ? (
                  dashboardData.recentSolicitacoes.map((s) => (
                    <div key={s.id} className="flex justify-between border-b py-3 last:border-0 items-center">
                      ...
                      <div className="text-right">
                        <span className={`text-xs font-bold px-2 py-1 rounded-full ${s.status === 'Pendente' ? 'bg-yellow-100 text-yellow-700' :
